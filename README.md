...
🟢 最新版本：v3.2，會自動判斷是否需要進行即時網路搜尋，無須再手動輸入關鍵字 (使用新版 Gemini 2.5 Flash-Lite 小型模型進行自動判斷)

較舊版本：
- v3：具備完整對話記憶、Google 搜尋及 YouTube 搜尋功能、OCR 圖片辨識
...
🔍 智慧語意搜尋：AI 會自動判斷是否需要進行即時網路搜尋，或直接使用資料庫知識來回答
...
v3.2 版起，AI 會自動判斷是否觸發即時搜尋，並回傳整理後的資訊摘要，無須使用者手動輸入關鍵字觸發。
